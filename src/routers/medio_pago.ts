...
/** Tabla: medio_pago  |  Campos: id, nombre  (solo lectura) */
...
export default async function medio_pago(app: FastifyInstance) {
  ...
  app.get('/', async (_req: FastifyRequest, reply: FastifyReply) => {
    try {
      const [rows] = await db.query('SELECT id, nombre FROM medio_pago ORDER BY id ASC');
      reply.send({ ok: true, items: rows });
    } catch (err: any) {
      reply.code(500).send({ ok: false, message: 'Error al listar medio_pago', error: err?.message });
    }
  });
  ...
  app.get('/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      reply.code(500).send({ ok: false, message: 'Error al obtener medio_pago', error: err?.message });
    }
  });
}
