...
export default async function posiciones(app: FastifyInstance) {
  ...
  app.post('/', async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (err?.errno === 1062) {
        return reply
          .code(409)
          .send({ ok: false, message: 'La posición ya existe' });
      }
  ...
  app.put('/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const [result]: any = await db.query(
        'UPDATE posiciones SET ? WHERE id = ?',
        [changes, id]
      );
  ...
  app.delete('/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const [result]: any = await db.query(
        'DELETE FROM posiciones WHERE id = ?',
        [id]
      );
      ...
      // Si tienes FK en jugadores.posicion_id con RESTRICT, puedes capturar 1451 aquí
      if (err?.errno === 1451) {
        return reply.code(409).send({
          ok: false,
          message:
            'No se puede eliminar: hay jugadores vinculados a esta posición.',
