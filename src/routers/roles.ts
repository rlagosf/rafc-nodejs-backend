...
/** Tabla: roles | Campos: id, nombre (solo lectura) */
...
export default async function roles(app: FastifyInstance) {
  ...
  app.get('/', async (_req: FastifyRequest, reply: FastifyReply) => {
    try {
      const [rows] = await db.query('SELECT id, nombre FROM roles ORDER BY id ASC');
      reply.send({ ok: true, items: rows });
    } catch (err: any) {
      reply.code(500).send({ ok: false, message: 'Error al listar roles', error: err?.message });
    }
  });
  ...
  app.get('/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      reply.code(500).send({ ok: false, message: 'Error al obtener rol', error: err?.message });
    }
  });
}
